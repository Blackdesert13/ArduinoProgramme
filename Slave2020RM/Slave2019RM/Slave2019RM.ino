/*
 ...
 Author:	Jürgen
*/
/*Slave-test befindet sich momentan in Entwicklung , zum Test der RM-Befehle auf dem PC-PGM
R�ckmeldung eingebaut noch nicht getestet,
vorl�ufig �ber Serial gesendet
*/
//#include <Servo.h>
#include <Wire.h>
const byte ArdNr = 12;//ArduinoNummer
unsigned long naechsterTestWechsel = 0;
int TestWechselPeriode;
...
void loop(){
	...
	{
		//RueckmeldungSlaveUSB();//alt
		USBRMMesswerteSenden();;
...
void USBRMMesswerteSenden() {//wird aktiviert über Befehl 80
	if (RMAnalyse < 255) {
		byte AnzUnterSW = 0;
		for (int i = 0; i < 10; i++) { 
			if (RMMessWert[RMAnalyse][i] < RMSchwelle) { AnzUnterSW++; };
		}
		Serial.write(RMMessWert[RMAnalyse][9]);
		Serial.write(RMMessWert[RMAnalyse][8]);
		Serial.write(RMMessWert[RMAnalyse][7]);
		Serial.write(RMMessWert[RMAnalyse][6]);
		Serial.write(RMMessWert[RMAnalyse][5]);
		Serial.write(RMMessWert[RMAnalyse][4]);
		Serial.write(RMMessWert[RMAnalyse][3]);
		Serial.write(RMMessWert[RMAnalyse][2]);
		Serial.write(RMMessWert[RMAnalyse][1]);
		Serial.write(RMMessWert[RMAnalyse][0]);
		...
		Serial.write(RMBefehl[2]);
		Serial.write(10);
		Serial.write(AnzUnterSW);
		Serial.write(RMSchwelle);
		Serial.write(RMAnalyse);
		Serial.write(ArdNr);
	}
}
...
void BefehlsAusfuehrung(byte Befehl[5])
{
	...
	switch (Befehl[1])
	{
		...
		case 80:
		{//Nummer der RM zur Analyse über USB
			RMAnalyse = Befehl[2];
			break;
		}
		case 81:
...
void RMTestfuerMoBS_USB() {
	if (millis() > naechsterTestWechsel) {
		...
		int summe = RMBefehl[0] + RMBefehl[1] + RMBefehl[2] + RMBefehl[3];
		RMBefehl[4] = lowByte(summe);
		summe = RMBefehl[5] + RMBefehl[6] + RMBefehl[7] + RMBefehl[8];
		RMBefehl[9] = lowByte(summe);
		Serial.write(RMBefehl, 10);
		naechsterTestWechsel += TestWechselPeriode;	
	}
}